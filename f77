...
Supported extensions: 
  .f, .f77  - Fortran 77 source
  .F        - Fortran 77 source with C preprocessor directives
...
for input_file in "${INPUT_FILES[@]}"; do
    ...
    if [ "$extension" == "F" ]; then
        if ! command -v "$CPP" >/dev/null 2>&1; then
            error_msg "preprocessing requested (.F file) but '$CPP' not found."
            exit 127
        fi
    ...
    if [ "$extension" == "f77" ]; then
        temp_f77_conv=$(mktemp --suffix=.f)
        TEMP_FILES+=("$temp_f77_conv")
        if ! cp "$input_file" "$temp_f77_conv"; then
             error_msg "failed to copy '$input_file'"
             exit 1
        fi
        f2c_input="$temp_f77_conv"
    fi

    LOGFILE=$(mktemp)
    TEMP_FILES+=("$LOGFILE")

    f2c "${F2C_OPTS[@]}" "$f2c_input" > "$LOGFILE" 2>&1
    rc=$?
